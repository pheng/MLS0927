...
/**
 * 当年到期贷款收回率情况统计表（按行业分类）
 */
public class LoanRate4Report extends BaseReport {
	...
	public LoanRate4Report(Context context) {
		super(context);
		this.context = context;
		this.setReportType(ReportType.LoanRate4);
	...
	class MyAdapter extends BaseAdapter {
		...
		public View getView(int position, View contentView, ViewGroup vg) {
			...
			if (contentView == null) {
				synchronized (context) {
					...
					View layout = LayoutInflater.from(context).inflate(
							R.layout.listview_report_loan_rate4, null);
					...
					headSrcrollView
							.AddOnScrollChangedListener(new OnScrollChangedListenerImp(
									scrollView1));
					holder.tvIndustryName = (TextView) layout
							.findViewById(R.id.t_industryname);
			...
			holder.tvIndustryName.setText(map.get(INDUSTRYNAME).toString());
		...
		handler.postDelayed(new Runnable() {
			public void run() {
				if (isNeedUpdate) {
					...
					final DoFetchThread doFetch = new DoFetchThread("RP0011", handler,jsonReq);
