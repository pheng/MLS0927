...
/**
 * 当年到期贷款收回率情况统计表（按贷款方式）
 */
public class LoanRate3Report extends BaseReport {
	...
	public LoanRate3Report(Context context) {
		super(context);
		this.context = context;
		this.setReportType(ReportType.LoanRate3);
	...
	class MyAdapter extends BaseAdapter {
		...
		public View getView(int position, View contentView, ViewGroup vg) {
			...
			if (contentView == null) {
				synchronized (context) {
					...
					View layout = LayoutInflater.from(context).inflate(
							R.layout.listview_report_loan_rate3, null);
					...
					headSrcrollView
							.AddOnScrollChangedListener(new OnScrollChangedListenerImp(
									scrollView1));
					holder.tvVouchName = (TextView) layout
							.findViewById(R.id.t_vouchname);
			...
			holder.tvVouchName.setText(map.get(VOUCHNAME).toString());
		...
		handler.postDelayed(new Runnable() {
			public void run() {
				if (isNeedUpdate) {
					...
					final DoFetchThread doFetch = new DoFetchThread("RP0010", handler,jsonReq);
	...
	private List<Map<String, Object>> parseJSON(String json) {
		...
		try {
			...
			for (int i = 0; i < array.length(); i++) {
				...
				map.put(VOUCHNAME, data.opt(VOUCHNAME));
				for (int j = 1; j <= 3; j++) {
